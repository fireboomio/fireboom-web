...
type InputArgViewProps = {
  ...
  parentField: Field
  modifyFields: (fields: ReadonlyArray<ObjectFieldNode>, commit: boolean) => DocumentNode | null
...
class InputArgView extends React.PureComponent<InputArgViewProps, {}> {
  ...
  _setArgValue = (
    ...
    const handleable =
      isLeafType(argType) || settingToVariable || settingToNull || settingToLiteralValue
    ...
    const newDoc = this.props.modifyFields(
      (selection.fields || []).map(field => {
        ...
        const newField = isTarget ? { ...field, value: value } : field
        return newField
      }),
      options
    )
...
class ArgView extends React.PureComponent<ArgViewProps, ArgViewState> {
  ...
  _setArgValue = (
    ...
    return this.props.modifyArguments(
      (selection.arguments || []).map(a => (a === argSelection ? { ...a, value: value } : a)),
      options
    )
  }
...
type ScalarInputProps = {
  ...
  argValue: ValueNode
  setArgValue: (
    arg0: React.SyntheticEvent<any> | VariableDefinitionNode,
    commit: boolean
  ) => DocumentNode | null
  onRunOperation: (arg0: void) => void
  styleConfig: StyleConfig
...
type AbstractArgViewProps = {
  ...
  setArgFields: (fields: ReadonlyArray<ObjectFieldNode>, commit: boolean) => DocumentNode | null
  ...
  render() {
    ...
    const variablize = () => {
      ...
      */
      ...
      const conflictingNameCount = (this.props.definition.variableDefinitions || []).filter(
        varDef => varDef.variable.name.value.startsWith(baseVariableName)
      ).length
      let variableName
      ...
      let subVariableUsageCountByName: Record<string, number> = {}
      ...
      const newlyUnusedVariables = Object.entries(subVariableUsageCountByName) // $FlowFixMe: Can't get Object.entries to realize usageCount *must* be a number
      ...
      if (variable) {
        ...
        if (newDoc) {
          ...
          const newOperation = { ...targetOperation, variableDefinitions: newVariableDefinitions }
          ...
          const finalDoc = { ...newDoc, definitions: newDefinitions }
...
class FragmentView extends React.PureComponent<FragmentViewProps, {}> {
  ...
  _getSelection = (): FragmentSpread | null | undefined => {
...
> {
  ...
  _modifySelections = (
    ...
    if (operationDef.kind === 'FragmentDefinition') {
      newOperationDef = {
        ...operationDef,
        selectionSet: { ...operationDef.selectionSet, selections }
      }
  ...
  /// Remove
  // _onOperationRename = event => this.props.onOperationRename(event.target.value)
...
> {
  ...
  render() {
    ...
    const styleConfig = {
      ...
      styles: this.props.styles ? { ...defaultStyles, ...this.props.styles } : defaultStyles
    }
    ...
    //     }
    //   })
    //   return { ...parsedQuery, definitions: newDefinitions }
    // }
    ...
    //   const newOperationDef = { ...parsedQuery, definitions: newDefinitions }
    //   this.setState({
    //     operationToScrollTo: `${kind}-${newOperationName}`
    //   })
    ...
    //           }
    //           style={{ ...styleConfig.styles.buttonStyle, height: '22px', width: '22px' }}
    //         >
    //           <span>+</span>
    //         </button>
    //       </form>
    ...
    const externalFragments =
      ...
      this.props.externalFragments.reduce((acc, fragment) => {
        if (fragment.kind === 'FragmentDefinition') {
          ...
          return { ...acc, [fragmentTypeName]: newFragmentsForType }
        }
    ...
    return (
      ...
      >
        ...
        <ExplorerFilter
          ...
          onSeletedDataSource={this._onDataSourceChange}
          dataSourceList={this.props.dataSourceList}
          setSelectedType={this._onChangeType}
        ...
        <div
          className="flex-1 doc-explorer-contents over-x-auto overflow-y-auto"
          style={{
            padding: '0px'
        ...
        >
          <ErrorBoundary>
            <Explorer {...this.props} {...this.state} />
          </ErrorBoundary>
        </div>
      </div>
    )
