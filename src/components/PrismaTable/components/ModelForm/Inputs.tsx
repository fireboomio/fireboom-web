...
import { DatePicker, Form, FormItemProps, Input, InputNumber, message, Modal, Radio, Select } from 'antd';
...
export const FormItem = ({ label, children, name, required, initialValue }: SelfFormItemProps & FormItemProps) => {
  return (
    <Form.Item className="w-full" label={label} name={name} rules={[ { required } ]} initialValue={initialValue}>
      {children}
    </Form.Item>
  );
};
...
export const Default = ({ field: { name, required, title }, disabled, initialValues }: Props) => (
...
const Datetime = ({ field: { name, required, title }, disabled, initialValues }: Props) => {
  ...
  return (
    <FormItem label={title} name={name} required={required} initialValue={initialDatetime}>
      <DatePicker disabled={disabled} className="w-full" showTime
...
const Enum = ({ field: { name, required, title, type }, disabled, initialValues }: Props) => {
  const { schema: { enums } } = useTableSchema();
    ...
    <FormItem label={title} name={name} required={required} initialValue={initialValues[name]}>
      <Select allowClear disabled={disabled}>
        {options.map((enumValue, idx) => (
          <Select.Option key={idx} value={enumValue}>{enumValue}</Select.Option>
        ))}
      </Select>
    </FormItem>
  );
...
const Object = ({ field: { name, required, title, type }, namespace, disabled, initialValues }: Props) => {
  const form = useFormInstance();
  const initialObjectValue = initialValues[name] as { [key: string]: any };
  ...
  const { schema: { models } } = useTableSchema();
  ...
  return (
    <>
      <Form.Item label={title} name={name} style={{ display: 'none' }}
                 rules={[ { required: required }, { validator: handleValidateObject } ]}>
        {<Input/>}
      </Form.Item>
      <FormItem label={title} required={required} className="w-full" noStyle>
        <Search disabled={disabled} onKeyDown={undefined} onPressEnter={undefined} readOnly
                allowClear placeholder="搜索并关联记录" value={displayValue}
                enterButton onSearch={() => setConnectModalVisible(true)}/>
      </FormItem>
      <Modal width={1200} title={`关联 ${type} 表数据`} visible={connectModalVisible}
             footer={false} destroyOnClose
             onCancel={() => setConnectModalVisible(false)}>
        <DynamicTable model={type} namespace={namespace} usage="connection" handleConnect={handleConnect}
                      initialFilters={filters} updateInitialFilters={setFilters} connectedRecord={initialObjectValue}
