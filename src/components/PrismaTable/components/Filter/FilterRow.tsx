...
import { Enums, SchemaModel } from '@paljs/types';
import { DatePicker, Input, InputRef, Select } from 'antd';
import moment from 'moment';
import { useEffect, useRef } from 'react';
import { useImmer } from 'use-immer';

import { FieldType, filterOperators } from '@/components/PrismaTable/components/Filter/FilterHelper';
import language from '@/components/PrismaTable/libs/language';
import { FilterState, FilterValueType } from '@/components/PrismaTable/libs/types';

const { Option } = Select;

const getFilterOperators = (type: FieldType) => {
  return Object.keys(filterOperators).filter(o => !filterOperators[o].notExistsInType.includes(type));
};
...
const FilterRow = ({ enums, filter, currentModel, models, deleteFilter, updateFilter }: FilterRowProps) => {
  ...
  const [ relationFieldNameSelectValue, setRelationFieldNameSelectValue ] = useImmer<string | undefined>(undefined);

  const getRelationFields = () => {
    const relation = currentModel.fields.find(f => f.name === fieldNameSelectValue);
    if (relation?.kind === 'object') {
      return models.find(m => m.id === relation.type)?.fields.filter(f => f.filter && f.kind !== 'object');
    }
  };

  const selectedField = fields.find(f => f.name === fieldNameSelectValue) ?? filter.field;
  useEffect(() => {
    setRelationFieldNameSelectValue(filter.relationField?.name ?? getRelationFields()?.map(f => f.name)[0]);
  ...
  const enumOptions = fieldType === 'Enum' ? enums.find(e => e.name === relationFieldNameSelectValue)?.fields ?? [] : [];
  ...
  useEffect(() => {
    ...
    if ([ 'Int', 'BigInt', 'Decimal', 'Float' ].includes(fieldType)) {
      filterValue = [ 'in', 'notIn' ].includes(operatorSelectValue) ?
        (String(filterValue)).split(',').filter(v => v).map(v => Number(v))
        : Number(filterValue);
    }
    if ('String' === fieldType) {
      filterValue = [ 'in', 'notIn' ].includes(operatorSelectValue) ?
        (String(filterValue)).split(',').filter(v => v)
        : filterValue;
    }
    updateFilter({
      field: selectedField,
  ...
  return (
    <div className="w-full flex flex-row items-baseline gap-3">
      <div className="w-468px flex flex-row gap-2 justify-between mb-2">
        <Select dropdownMatchSelectWidth={false} className="w-1/3" value={fieldNameSelectValue || fields[0].name}
        ...
        {relationFieldsOptions &&
          <Select dropdownMatchSelectWidth={false} className="w-1/3"
        ...
        <Select dropdownMatchSelectWidth={false} className="w-1/3" value={operatorSelectValue || operators[0]}
                onSelect={setOperatorSelectValue}>
          {operators.map((op, idx) => (
            <Option key={idx} value={op}>{op}</Option>
          ))}
        </Select>
        <FilterValue enums={enumOptions} handleValueChange={setInputValue} inputValue={inputValue} fieldType={fieldType}/>
      </div>
...
const FilterValue = ({ inputValue, handleValueChange, fieldType, enums }: FilterValueProps) => {
  const [ inputString, setInputString ] = useImmer<string>((typeof inputValue === 'object') ? (inputValue as string[]).join(',') : (inputValue as string));

  useEffect(() => {
    setInputString((typeof inputValue === 'object') ? (inputValue as string[]).join(',') : (inputValue as string));
  }, [inputValue]);
  ...
  switch (fieldType) {
    case 'DateTime': {
      ...
      return (
        <DatePicker defaultValue={defaultDate} className="w-1/3"
                    onChange={(_: unknown, dateString: string) => handleValueChange(dateString)}/>
      );
    }
    case 'Boolean':
      return (
        <Select dropdownMatchSelectWidth={false} className="w-1/3" onSelect={(value: boolean) => handleValueChange(value)}>
    ...
    default:
      return <Input ref={inputRef} autoFocus value={inputString} onChange={handleInputChange} onBlur={handleInputBlur}/>;
  }
};
