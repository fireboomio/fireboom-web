...
export const getGraphqlQuery = (
  ...
  if (findUnique) {
    return tag`
      query ${graphQLNamespacePrefix(namespace)}findUnique${modelName}($where: ${graphQLNamespacePrefix(namespace)}${modelName}WhereUniqueInput!) {
    ...
    return tag`
      query ${graphQLNamespacePrefix(namespace)}findMany${modelName}(
        ...
        ${graphQLNamespacePrefix(namespace)}findMany${modelName}(where: $where, orderBy: $orderBy, skip: $skip, take: $take) {
          ${fields}
        }
...
export const getGraphqlMutation = (
  ...
  switch (mutation) {
    case 'create':
      return tag`mutation ${graphQLNamespacePrefix(namespace)}createOne${model}($data: ${graphQLNamespacePrefix(namespace)}${model}CreateInput!) {
              ${graphQLNamespacePrefix(namespace)}createOne${model}(data: $data) {
                ${modelObject?.idField || allScalar(modelObject)}
              }
            }`;
    case 'delete':
      return tag`mutation deleteOne${model} ($where: ${graphQLNamespacePrefix(namespace)}${model}WhereUniqueInput!) {
  ${graphQLNamespacePrefix(namespace)}deleteOne${model} (where: $where) {
    ${modelObject?.idField || allScalar(modelObject)}
  }
}`;
    case 'update':
      return tag`mutation ${graphQLNamespacePrefix(namespace)}updateOne${model} ($where: ${graphQLNamespacePrefix(namespace)}${model}WhereUniqueInput!, $data: ${graphQLNamespacePrefix(namespace)}${model}UpdateInput!) {
