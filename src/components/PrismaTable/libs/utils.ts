...
import type { TableData } from '@/components/PrismaTable/libs/types'
...
export const getTableDataFromGraphQLResp = (
  model: string,
  dataRecord: any,
  namespace?: string
): TableData => {
  return {
    tableData: dataRecord
      ? (dataRecord[`${namespace ? namespace + '_' : ''}findMany${model}`] as Record<
          string,
          any
        >[]) ?? []
      : [],
    // tableDataTotalCount: dataRecord ? dataRecord[`${namespace ? namespace + '_' : ''}findMany${model}Count`] as number ?? 0 : 0,
    tableDataTotalCount: dataRecord
      ? (dataRecord[`${namespace ? namespace + '_' : ''}aggregate${model}`] as AggregateCount)
          ._count._all ?? 0
      : 0
  }
}
