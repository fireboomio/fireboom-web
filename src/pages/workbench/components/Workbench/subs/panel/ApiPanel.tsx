import { Dropdown, Modal, Popconfirm, Tooltip, message } from "antd";
...
export default function ApiPanel(props: Omit<SidePanelProps, "title">) {
	...
	const [treeData, setTreeData] =
		useState<ApiDocuments.fileloader_DataTree[]>();
	...
	const executeWrapper = useCallback(
		<T extends Function>(fn: T, skipMutate = false): T =>
			(async (...args: any) => {
				const hide = message.loading(
					intl.formatMessage({ defaultMessage: "执行中" }),
				);
				try {
					await fn(...args);
	...
	const handleBatchDelete = executeWrapper(
		async (nodes: ApiDocuments.fileloader_DataTree[]) => {
			modal.confirm({
				title: intl.formatMessage({
					defaultMessage: "是否确认删除选中的API？",
				}),
				onOk: executeWrapper(async () => {
					...
					pathList.forEach((path) =>
						localStorage.removeItem(`_api_args_${path}`),
					);
	...
	const handleDelete = executeWrapper(
		async (node: ApiDocuments.fileloader_DataTree) => {
			if (node.isDir) {
	...
	const handleAddNode = executeWrapper(async (path: string, isDir: boolean) => {
		if (isDir) {
			await requests.post(`/vscode/createDirectory`, {
				uri: `${operationStorePath}/${path}`,
			});
			await mutateApi();
		} else {
	...
	const handleRenameNode = executeWrapper(
		async (node: ApiDocuments.fileloader_DataTree, newName: string) => {
			...
			if (node.isDir) {
				...
				await requests.post(`/operation/rename`, {
					src: oldPath,
					dst: newPath,
					overload: false,
				});
	...
	const handleMove = executeWrapper(
		async (
			...
			const newPath = `${
				dropNode?.path ? `${dropNode.path}/` : ""
			}${dragNode.path?.split("/").pop()}`;
				...
				await requests.post(
					...
					{ src: oldPath, dst: newPath, overload: confirm },
					{
						onError: async ({ code, result }) => {
							if (code === "20000000") {
								await confirmExecute(result, () =>
									handleMove(dragNode, dropNode, true),
								);
							}
						},
					},
	...
	const titleRender = (nodeData: ApiDocuments.fileloader_DataTree) => {
		...
		const isCustomDir =
			nodeData.isDir &&
			["function", "proxy"].includes(_parent?.name ?? nodeData.name);
		...
		return (
			<div className={`${styles.treeItem} ${itemTypeClass}`}>
				...
				>
					{nodeData.extra?.liveQueryEnabled ? (
			...
			const menu: ItemType[] = [
				{
					...
					disabled: !deepList.some(
						(x) => !x.isDir && !x.extra.enabled && !x.extra.invalid,
					),
			...
			action={
				<>
					...
					<Tooltip title={intl.formatMessage({ defaultMessage: "全局设置" })}>
						<div
							className={styles.headerConfig}
							onClick={() => {
								navigate("/workbench/setting/global-api-setting");
							}}
						/>
					</Tooltip>
						...
						if (!saved) {
							const confirmed = await modal.confirm({
								content: intl.formatMessage({
									defaultMessage: "当前 API 未保存，是否保存",
								}),
							...
							if (confirmed) {
								message.open({
									type: "loading",
									content: intl.formatMessage({ defaultMessage: "自动保存中" }),
								});
								await autoSave();
								message.destroy();
							}
					...
					if (name && onValidateName(name, isDir)) {
						...
						if (_parent?.name === "function" || _parent?.name === "proxy") {
							if (await checkSupport(_parent.name)) {
								const resp = await requests.get<null, ApiDocuments.Sdk>(
									"/sdk/enabledServer",
								);
								if (resp?.extension) {
									if (
										await vscode.show(
											`${resp.outputPath}/${parent?.path}/${name}${resp.extension}`,
										)
									) {
										message.info(
											intl.formatMessage({
												defaultMessage:
													"数据源创建成功，请在编辑完成后重启钩子服务",
											}),
										);
									}
								}
					...
					<div className={styles.createRowWrapper}>
						...
						>
							<span
								...
								onClick={() => fileTree.current.addItem(false)}
							>
							...
							<span
								className={styles.btn}
								onClick={() => navigate(`/workbench/apimanage/crud`)}
							>
						...
						<Tooltip title={intl.formatMessage({ defaultMessage: "测试" })}>
							...
							>
								<img
									alt=""
									src={`${import.meta.env.BASE_URL}assets/icon/graphql2.svg`}
								/>
							</div>
						</Tooltip>
					</div>
...
function convertToTree(
	data: ApiDocuments.fileloader_DataTree[] | null,
	lv = "0",
) {
	if (!data) return [];
