...
const DirectiveDescription = ({ name, title, code }: DirectiveDescriptionProps) => {
  ...
  return (
    <div className="flex flex-col h-full p-2.5 overflow-y-hidden">
      ...
      <code className="bg-[#F8F9FD] flex-1 mt-2 text-default text-xs px-2 pt-4.5 pb-2 leading-5 block relative overflow-hidden">
        <pre className="h-full my-0 w-full overflow-auto">{code}</pre>

        <CopyOutlined
          className="cursor-pointer text-xs top-1.5 right-1.5 absolute"
          onClick={() => {
            copy(code)
