...
const ModelingWrapper = (props: { children: ReactNode }) => {
	...
	const [state, dispatch] = useReducer(
		modelingReducer,
		emptyPrismaSchemaContextState.state,
	);
	...
	const loadDataSource = useCallback(async () => {
		...
		if (dataSources.length > 0 && paramName) {
			...
			const hide = message.loading(
				intl.formatMessage({ defaultMessage: "加载中" }),
				0,
			);
			await fetchAndSaveToPrismaSchemaContext(
				paramName,
				dispatch,
				dataSources,
				paramNameRef,
	...
	useEffect(() => {
		...
		if (
			state.blocks.length &&
			!state.blocks.find((b) => ["model", "enum"].includes(b.type))
		) {
	...
	const handleSetInEdit = (flag: boolean) => {
		...
		if (inEdit) {
			if (Object.keys({ ...newMap, ...delMap, ...editMap }).length > 0) {
				message.error(
					intl.formatMessage({ defaultMessage: "请先保存或取消编辑" }),
				);
				return;
			}
		}
