...
import type { GraphQLInputField, GraphQLInputType } from 'graphql'
...
type PropertiedJSON6 = JSONSchema6 & {
  properties: Record<string, JSONSchema6>
}
...
type Definitions = Record<string, JSONSchema6Definition>
...
function renderTypeToString(t: GraphQLInputType | GraphQLInputField, useMarkdown?: boolean) {
  const into: string[] = []
  if (useMarkdown) {
    text(into, '```graphql\n')
  }
...
const scalarTypesMap: Record<string, JSONSchema6TypeName> = {
  Int: 'integer',
  String: 'string',
  Float: 'number',
  ID: 'string',
...
function getJSONSchemaFromGraphQLType(
  ...
  // append descriptions
  if ('description' in type && !isScalarType(type) && type.description && !definition.description) {
