...
import type { NativeTypeConstructors } from './prisma-fmt/nativeTypes'
import nativeTypeConstructors from './prisma-fmt/nativeTypes'
...
export function* getBlocks(lines: string[]): Generator<Block, void, void> {
  ...
  for (const [key, item] of lines.entries()) {
    ...
    if (
      allowedBlockIdentifiers.some(identifier => item.startsWith(identifier)) &&
      item.includes('{')
    ) {
...
export function getAllPreviewFeaturesFromGenerators(
  lines: string[]
...
export function getFieldsFromCurrentBlock(
  lines: string[],
  block: Block,
  position?: Position
): string[] {
